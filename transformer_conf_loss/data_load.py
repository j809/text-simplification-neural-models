...
def load_vocab(vocab_fpath, ppdb_rules_path, get_rules = False):
    ...
    rules = dict()
    if get_rules:
        with open(ppdb_rules_path,"r") as file:
            ...
            for line in lines:
                ...
                count += 1
                try:
                    if arr[3] in token2idx:
                        # print(count,"--->",arr[3], "--->", token2idx[arr[3]] )
                        rules[token2idx[arr[3]]] = arr[4]
                except IndexError:
                    pass
    return token2idx, idx2token, rules
...
def generator_fn(sents1, sents2, vocab_fpath):
    ...
    '''
    token2idx, _, _ = load_vocab(vocab_fpath, None, get_rules = False)
